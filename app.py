...
# ═══════════════════════════════════════════════════════════════════════════
# UI — Sidebar (filters, reviewer name, refresh)
# ═══════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("### 🔍 الفلاتر")
    reviewer_name = st.text_input("اسم المراجع", value=st.session_state.get("reviewer_name", ""))
    st.session_state["reviewer_name"] = reviewer_name

    st.divider()
    search_query = st.text_input("ابحث بالكلمة أو المعنى", value="")
...
if search_query.strip():
    ...
    mask = (
        filtered.get(COL_LEMMA, pd.Series([""]*len(filtered))).astype(str).str.contains(q, na=False) |
        filtered.get(COL_NONDIAC, pd.Series([""]*len(filtered))).astype(str).str.contains(q, na=False) |
        filtered.get(COL_DEFINITION, pd.Series([""]*len(filtered))).astype(str).str.contains(q, na=False)
    )
...
def render_detail(row: pd.Series):
    ...
    with col_info:
        ...
        if not reviewer_name.strip():
            st.warning("اكتبي اسم المراجع في الشريط الجانبي قبل إجراء أي قرار.")
        ...
        with tab_reject:
            ...
            disabled = (not reviewer_name.strip()) or (not reason.strip())
            help_txt = None
            if not reviewer_name.strip():
                help_txt = "اكتبي اسم المراجع أولاً"
            elif not reason.strip():
                help_txt = "اكتبي سبب الرفض"
